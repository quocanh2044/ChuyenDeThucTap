...
// Đảm bảo URL này khớp với Server của bạn để load ảnh cục bộ (nếu có)
...
const AdminDashboard: React.FC = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const mappedMovies = (revenueData || []).map((m: any) => {
        ...
        // XỬ LÝ HÌNH ẢNH: Ưu tiên trường 'image' từ DB
        const rawImage = m.image || m.poster;
        let finalPoster = FALLBACK_POSTER;

        if (rawImage) {
          if (rawImage.startsWith('http')) {
            finalPoster = rawImage;
          } else {
            // Nếu là tên file (vd: latmat7.jpg), nối với đường dẫn public của server
            finalPoster = `${BASE_URL}/uploads/${rawImage}`;
          }
        }
        ...
        return {
          ...
          poster: finalPoster, // Gán lại vào thuộc tính poster để hiển thị
          sold: ticketsSold,
  ...
  const handleAddMovie = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMovieForm({ title: "", poster: "", price: 80000, duration: 120 });
      fetchData();
    } catch (err: any) {
    ...
    <div className="flex min-h-screen bg-cinema-900 text-white font-sans">
          ...
          {["dashboard", "movies", "users"].map((tab) => (
            <button
              ...
              className={`w-full text-left px-5 py-4 rounded-2xl transition-all font-bold ${
                activeTab === tab ? "bg-yellow-500 text-black shadow-lg shadow-yellow-500/20" : "hover:bg-white/5 text-gray-400"
              }`}
            >
          ...
          <div className="space-y-10 animate-in fade-in duration-500">
            ...
            <div className="bg-cinema-800 p-8 rounded-[40px] border border-white/5 shadow-2xl">
                ...
                <div className="space-y-6">
                    ...
                    {movies.length === 0 && <p className="text-center text-gray-500 italic py-10">Chưa có giao dịch thành công.</p>}
                </div>
            </div>
          </div>
        )}
        ...
        {activeTab === "movies" && (
          <div className="animate-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center mb-10">
              ...
              <button onClick={() => setShowMovieForm(true)} className="bg-green-600 hover:bg-green-500 px-8 py-3 rounded-2xl font-black text-sm shadow-lg shadow-green-600/20">
                + THÊM PHIM
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-8">
              {movies.map((m) => (
                <div key={m.id} className="bg-cinema-800 rounded-3xl overflow-hidden border border-white/5 flex flex-col group transition-all">
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between">
                    <div>
                        <h4 className="font-bold text-lg line-clamp-2 mb-4 h-12 italic tracking-tight">{m.title}</h4>
                        <div className="bg-cinema-900/50 p-4 rounded-2xl mb-6 space-y-1">
                            <p className="text-sm font-bold">Đã bán: {m.sold} vé</p>
                            <p className="text-sm font-bold text-yellow-500">Tiền: {m.revenue.toLocaleString()}đ</p>
                        </div>
                    </div>
                    <button onClick={() => handleDeleteMovie(m.id)} className="w-full bg-red-600/10 hover:bg-red-600 text-red-500 hover:text-white py-3.5 rounded-xl font-bold text-[10px] uppercase tracking-widest transition-all">
                        Xóa phim này
                    </button>
                  </div>
                </div>
                ...
                {users.map((u) => (
                  <tr key={u.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="p-6 text-right">
                      {u.role !== "admin" && (
                        <button onClick={() => handleDeleteUser(u.id)} className="text-red-500 font-bold text-xs uppercase hover:underline underline-offset-4">Xóa</button>
                      )}
                    </td>
                  </tr>
                ))}
      ...
      {showMovieForm && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md flex items-center justify-center z-50 p-6">
          <form onSubmit={handleAddMovie} className="bg-cinema-800 p-10 rounded-[40px] w-full max-w-lg space-y-6 border border-white/10 shadow-2xl">
            <h3 className="text-2xl font-black text-yellow-500 italic text-center uppercase tracking-widest">Cấu hình phim mới</h3>
            <div className="space-y-4">
                <input className="w-full p-4 bg-cinema-900 border border-white/5 rounded-2xl outline-none focus:border-yellow-500 transition-all" placeholder="Tiêu đề phim" value={movieForm.title} onChange={(e) => setMovieForm({ ...movieForm, title: e.target.value })} required />
                <input className="w-full p-4 bg-cinema-900 border border-white/5 rounded-2xl outline-none focus:border-yellow-500 transition-all" placeholder="URL hoặc Tên file ảnh (vd: movie.jpg)" value={movieForm.poster} onChange={(e) => setMovieForm({ ...movieForm, poster: e.target.value })} required />
                <div className="grid grid-cols-2 gap-4">
                    <input type="number" className="p-4 bg-cinema-900 border border-white/5 rounded-2xl outline-none focus:border-yellow-500 transition-all" value={movieForm.price} onChange={(e) => setMovieForm({ ...movieForm, price: Number(e.target.value) })} />
                    <input type="number" className="p-4 bg-cinema-900 border border-white/5 rounded-2xl outline-none focus:border-yellow-500 transition-all" value={movieForm.duration} onChange={(e) => setMovieForm({ ...movieForm, duration: Number(e.target.value) })} />
                </div>
            </div>
            <div className="flex gap-4 pt-4">
              <button type="button" onClick={() => setShowMovieForm(false)} className="flex-1 bg-white/5 py-5 rounded-2xl font-bold">Hủy</button>
              <button type="submit" className="flex-1 bg-yellow-500 text-black py-5 rounded-2xl font-black">Lưu phim</button>
            </div>
          </form>
        </div>
      )}
